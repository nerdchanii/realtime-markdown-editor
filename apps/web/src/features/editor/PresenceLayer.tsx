...
export function PresenceLayer({ members }: { members: readonly PresenceMember[] }) {
  return (
    <div aria-label="Remote presence" data-testid="presence-layer" style={presenceLayerStyle}>
      {members.map((member) => (
        <span key={member.id} style={presenceRailStyle(member)}>
          <span aria-hidden="true" style={presenceCaretStyle(member.color)} />
          <span aria-hidden="true" style={presenceSelectionStyle(member)} />
          <span
