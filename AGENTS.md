...
- **UI**
  ...
  - 요소를 card 나 border 로 감싸지 않는다. 색은 theme token 으로만 쓴다. `pnpm ui:check` 가 검사한다.
...
pnpm check         # typecheck, lint, format:check, docs:check, ui:check, arch:check, test
pnpm docs:check    # 깨진 링크, ADR 형식, proposed ADR 인용 검사
pnpm ui:check      # UI 규칙 ratchet(테두리 박스, 하드코딩 색). baseline 은 줄이는 방향으로만 갱신
