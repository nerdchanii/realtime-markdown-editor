...
- **개정 (2026-09-26, [user])**: `DocumentState` 는 `meta` 에 두지 않고 Y.Doc 밖의 문서 레코드에 둔다. 근거와 세부는 [ADR-0017](0017-document-workflow-triggers-and-executor.md) §4 (accepted 2026-09-26)에 있다.
  - 정본 위치는 범위별 권위(ADR-0011)를 따른다. server 범위는 DB, local 범위는 기기 저장소(IndexedDB)의 문서 레코드다.
  - 상태 변경은 `ChangeDocumentState` use case 로만 한다. server 범위는 DB adapter 로 실행하고 권한 판정과 outbox 기록을 그곳에서 한다. local 범위는 local 저장 adapter 로 실행하며 서버나 계정이 필요 없다.
...
| 2026-09-27 | 개정문을 ADR-0017 §4 에 맞춰 범위별로 고쳤다. local 범위는 기기 저장소 레코드와 local adapter 를 쓴다(Codex 리뷰 반영) | user |
