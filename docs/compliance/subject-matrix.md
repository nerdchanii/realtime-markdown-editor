...
| --- | --- | --- | --- | --- | --- | --- |
| `CE-01-CONCURRENT-EDITING` | 2명 이상이 동일한 Markdown 문서를 동시에 편집할 수 있어야 한다. | 하나의 workspace document에서 여러 client가 수렴하는지 증명한다. | 두 browser session이 서로 다른 위치를 편집하고 manual refresh 없이 같은 내용으로 수렴한다. | `docs/product/editor/concurrent-editing.md` | `REQ-COLLAB-ENGINE-ADAPTER`, `REQ-WORKSPACE-DOCUMENT-SCOPE` | ADR-0001, ADR-0002 |
