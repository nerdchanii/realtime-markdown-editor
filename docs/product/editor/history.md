...
History는 ordinary autosave/sync와 intentional checkpoint를 구분하면서 읽을 수 있는 document state를 제공한다.

## 제품 범위

- Explicit checkpoint를 만들거나 노출한다.
- Author, timestamp, message, content state를 표시한다.
- 사용자가 previous document content를 inspect할 수 있게 한다.
- 모든 autosave를 user-facing revision처럼 취급하지 않는다.
- 현재 history path는 checkpoint 생성, 목록, metadata, read-only snapshot inspection이다.
...
- Checkpoint 생성은 server-resolved author membership, timestamp, message, snapshot artifact reference를 metadata로 저장한다.
...
- CRDT update replay, rendered HTML snapshot 저장, combined artifact format은 현재 제품 범위가 아니다. ADR-0003 V1은 checkpoint artifact를 inspectable Markdown snapshot과 artifact metadata로 둔다.
...
사용자가 checkpoint를 만들거나 선택하고, metadata를 본 뒤 read-only snapshot viewer에서 저장된 Markdown content를 확인한다. 다른 member session도 생성된 checkpoint를 normal product path에서 볼 수 있어야 한다.
