...
| UI-GAP-011 | Top bar workspace context | Top bar shows static organization/project labels.                         | Workspace/project switcher data, current context labels, and scoped navigation.         | `REQ-WORKSPACE-LIFECYCLE-MANAGEMENT`, `REQ-WORKSPACE-MEMBER-MANAGEMENT`                                      | open   |
...
| UI-GAP-017 | Auth seed account UI      | Auth screen exposes seed account shortcut and development-only copy.       | Production account UX without seed credentials or reviewer bootstrap language.          | `REQ-PRODUCTION-ACCOUNT-MANAGEMENT`, `REQ-DEV-LOCAL-PRODUCT-SEED-DATA`                                       | open   |
