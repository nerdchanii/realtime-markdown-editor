...
| UI-GAP-015 | Seed review context path   | App no longer contains the seed review context provider path.                                                                    | Removed seed review runtime provider and client usage.                                             | `REQ-DEV-LOCAL-PRODUCT-SEED-DATA`, `REQ-CE-ACCEPTANCE-TEST-DECOUPLING`    | closed |
| UI-GAP-016 | Workspace seed fallback    | Workspace normalization now falls back to an explicit empty navigation model instead of seeded nav data.                          | Keep product navigation sourced from API data, explicit empty states, or real errors.              | `REQ-WORKSPACE-HIERARCHY`, frontend product API follow-up                 | closed |
