...
| ---------- | ------------------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------ |
| UI-GAP-001 | Profile menu / settings   | Profile menu opens scoped User, Workspace, and Project settings with read-only identity data from the session/navigation model. | Full settings persistence and account/workspace management UX.                         | `REQ-PRODUCTION-ACCOUNT-MANAGEMENT`, `REQ-WORKSPACE-LIFECYCLE-MANAGEMENT`, `REQ-WORKSPACE-MEMBER-MANAGEMENT` | open   |
...
| UI-GAP-008 | Trash                     | Left sidebar shows Trash button but it does not yet call the available Trash list/restore APIs. | Product UI wiring for document Trash list and restore workflow.                        | `REQ-DOCUMENT-TRASH-RESTORE`                                                                                 | open   |
...
| UI-GAP-017 | Auth seed account UI      | Primary auth copy is product-shaped; development builds still expose labeled local seed account shortcuts. | Account creation/bootstrap and profile/deactivation mutations without seed credentials. | `REQ-PRODUCTION-ACCOUNT-MANAGEMENT`, `REQ-DEV-LOCAL-PRODUCT-SEED-DATA`                                       | open   |
