...
- [x] 검증 결과를 이 문서에 기록했다. (Typecheck and unit tests pass. UI handles `unauthenticated` and `no-workspace` correctly.)
- [x] 필요한 공식 문서 업데이트를 완료했다. (Requirements are linked, placeholders added for onboarding.)
- [x] 관련 CE/REQ evidence 또는 follow-up을 기록했다. (Added placeholder UI for `REQ-ACCOUNT-WORKSPACE-ONBOARDING`.)
