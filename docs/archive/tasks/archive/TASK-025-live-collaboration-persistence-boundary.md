...
owner: worker-019ddb0e-4360-7530-a250-fdb8fd2fb8b9
depends_on:
  - TASK-024
write_set:
  - apps/collab/**
...
- Verification:
  ...
  - `pnpm lint` -> pass.
- Worker reported an additional filesystem Yjs persist/reload smoke check passed.
- Main session added `apps/collab/.gitignore` for `.data/` runtime state generated by the
  filesystem adapter smoke path.
- Worker reported acceptance met and no unresolved TASK-025 blockers.
- Follow-up: retention/compaction for persisted live Yjs binary files remains future provider
  policy, not product revision artifact history.
