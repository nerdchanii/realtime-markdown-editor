...
| Document property | Markdown body 밖에 저장되는 구조화된 metadata. |
| DocumentState | `draft`, `review`, `saved` 같은 workflow-facing 상태. |
...
| Presence | cursor와 selection을 보여주는 임시 realtime awareness. |
...
| Collaboration artifact | sync/history에 필요한 provider-specific 또는 serialized document state. |
