...
| 2026-09-26 | `apps/editor` 는 React 19 + Vite + CodeMirror 6 + `y-codemirror.next` 로 만든다. local 문서 목록은 id 만 담는 index Y.Doc 에 두고, 제목은 각 문서에서만 얻는다(값마다 정본 하나) | G1 | agent:claude-code | pending | 2026-10-03 | 문서 수가 많아져 목록을 열 때 문서를 모두 여는 비용이 문제가 되면 | #15 PR |
| 2026-09-26 | 문서 제목은 본문의 첫 최상위 H1 이다. `meta.title` 과 제목 입력칸을 없애고, 제목은 type module 의 `title` projection 으로 계산한다. H1 이 없으면 "제목 없음"으로 표시한다 | G1 | user (첫 H1 을 제목으로) / agent:claude-code (저장하지 않는 projection 으로 구현) | user | - | 본문과 다른 제목이 필요해지면 | #15 PR |
