...
| 2026-09-25 | ADR-0013 기존 Yjs state migration 순서: Tiptap XmlFragment 직렬화 → `Y.Text "markdown"` → `markdownBody`. migration 전에 백업한다. client 버전 gate 로 옛 client 를 거절하고, legacy draft 의 미동기화 변경은 "복구된 초안"으로 보여준다. legacy 루트는 호환 기간 동안 읽기 전용으로 두고 이후에 제거한다 | G1 | agent:claude-code | pending | 2026-10-02 | migration 검증에서 내용 차이가 발견되면 | PR #9 |
