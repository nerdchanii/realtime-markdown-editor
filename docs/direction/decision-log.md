...
| 2026-09-26 | 협업 연결 token 은 HS256 JWT 이고 API 와 collab 이 공유하는 서명 값(`RME_COLLAB_TOKEN_SECRET`, 32자 이상)으로 서명한다. 값이 없으면 두 프로세스 모두 시작하지 않고, 고정 기본값은 두지 않는다. `pnpm dev` 는 실행마다 임의 값을 만든다 | G1 | agent:claude-code | pending | 2026-10-03 | collab 이 아닌 곳(에이전트 프로토콜 등)도 token 을 검증해야 하거나 서명 값 공유가 운영에서 부담이 되면 비대칭 키로 바꾼다 | 슬라이스 2 협업 연결 인증 PR |
| 2026-09-26 | 협업 연결 token TTL 기본값은 120초, 최대 900초다. 만료 판정에 clock skew 허용은 두지 않는다 | G1 | agent:claude-code | pending | 2026-10-03 | provider 재연결과 token 갱신 흐름을 붙일 때 TTL 이 너무 짧거나 서버 간 시계 차이가 문제가 되면 | 슬라이스 2 협업 연결 인증 PR |
| 2026-09-26 | 협업 세션 발급 응답(`POST /documents/:documentId/collaboration-sessions`)에 `connection { token, expiresAt, access }` 를 싣는다. collab 이 쓰는 internal 세션 조회 응답에는 token 을 싣지 않는다 | G1 | agent:claude-code | pending | 2026-10-03 | token 발급을 별도 endpoint 로 떼어야 할 이유가 생기면 | 슬라이스 2 협업 연결 인증 PR |
