...
| 2026-09-25 | ADR-0013 기존 Yjs state migration 순서: Tiptap XmlFragment 직렬화 → `Y.Text "markdown"` → `markdownBody`. migration 뒤 이전 루트를 제거하고, migration 전에 백업한다 | G1 | agent:claude-code | pending | 2026-10-02 | migration 검증에서 내용 차이가 발견되면 | PR #9 |
